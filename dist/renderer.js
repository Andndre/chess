...
class Renderer {
    ...
    drawPiece(piece) {
        ...
        let div = 1;
        if (rotate && flip) {
            ctx.translate((x / 2) * boxScale + boxScale, (y / 2) * boxScale + boxScale);
            ctx.rotate(Math.PI);
            sign = -1;
            div = 2;
        }
        // draw the piece
        ctx.drawImage(this.sprite, sx, sy, singleSpriteSize, singleSpriteSize, (x * boxScale * sign) / div, (y * boxScale * sign) / div, boxScale, boxScale);
        ctx.restore();
    }
}
