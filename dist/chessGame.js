...
export class ChessGame {
    constructor(canvas, fen) {
        ...
        ["", "webkit", "moz", "ms"].forEach((prefix) => document.addEventListener(prefix + "fullscreenchange", () => this.windowResized(this), false));
    }
    windowResized(obj = this) {
        const canvasSize = Math.min(window.innerHeight, window.innerWidth);
        const timersContainer = document.getElementsByClassName("timers")[0];
