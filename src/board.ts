class Board{
  ...
  draw(){
    ...
    if(selectedIndex != undefined){
      ...
      this.ctx.fillRect(
        x * Board.boxScale, y * Board.boxScale, 
        Board.boxScale, Board.boxScale
      );
    }
    ...
    for(let index of this.chess.availableMoves){
      ...
      if(!this.chess.board[index]){
        ...
        this.ctx.arc(
          ...
          y * Board.boxScale + (Board.boxScale >> 1), 
          Board.boxScale >> 3, 
          0, Math.PI * 2
          );
  ...
  drawPieces (){
    for (let i = 0; i < 8; i++){
      for (let j = 0; j < 8; j++){
        ...
        // highlight selected
        if (this.chess.board[index] == Piece.none) continue;
        this.drawPiece (this.chess.board[index]!, index);
      }
    }
  }

  drawPiece (piece: number, index: number){
    ...
    let sx = [
      ...
      Piece.knight, 
      Piece.rook, 
      Piece.pawn].indexOf (Piece.getType(piece)) * 213;
    let [x, y] = getCoords (index);
    this.ctx.drawImage (
      this.img, 
      sx,sy,213,213,
  ...
  static getClickedCoord(x: number, y: number): number[]{
    x = Math.floor(x / Board.boxScale);
    y = Math.floor(y / Board.boxScale);
    return [x, y];
  }

  static getCLickedIndex(x: number, y: number): number{
