...
class Chess {
    ...
    generateAvailableMoves(piece, index) {
            ...
            for (let i = 0; i < 2; i++) {
                for (let xOffset of range[i]) {
                    for (let yOffset of range[1 - i]) {
                        ...
                        let status = this.insertAvMove(index, result);
        ...
        // KING //
            ...
            if (x != 0)
                this.insertAvMove(index - 1, result);
            if (x != 7)
                this.insertAvMove(index + 1, result);
            if (y != 0)
                this.insertAvMove(index - 8, result);
            if (y != 7)
                this.insertAvMove(index + 8, result);
            if (y != 7 && x != 7)
                this.insertAvMove(index + 9, result);
            if (y != 0 && x != 7)
                this.insertAvMove(index - 7, result);
            if (y != 7 && x != 0)
                this.insertAvMove(index + 7, result);
            if (y != 0 && x != 0)
                this.insertAvMove(index - 9, result);
                ...
                if (y != 0) {
                    ...
                    if (x != 0) {
                        if (this.checkAvMoves(index - 7) === 'enemy') {
                            if (Piece.getType(this.board[index - 7]) == Piece.king)
                                this.check = index - 7;
                            result.push(index - 7);
                        }
                    }
                    if (x != 7) {
                        if (this.checkAvMoves(index - 9) === 'enemy') {
                            if (Piece.getType(this.board[index - 9]) == Piece.king)
                                this.check = index - 9;
                            result.push(index - 9);
                        }
                    }
